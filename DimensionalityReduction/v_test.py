...
def v_test(input_data, computed_data, list_vars,folder):
    ...
    for dim in range(pca_data.shape[0]):
        print'--- Dim : ', dim
        high_values[dim] = list(zip(*heapq.nlargest(3000, enumerate(pca_data[dim, :]), key=operator.itemgetter(1)))[0])
        low_values[dim] = list(zip(*heapq.nsmallest(3000, enumerate(pca_data[dim, :]), key=operator.itemgetter(1)))[0])
        output_high = numpy.zeros((prep_data.shape[0], 3000))
        output_low = numpy.zeros((prep_data.shape[0], 3000))
        ...
        for n_var in range(len(list_vars)):
            result = [list_vars[n_var],
                      ((numpy.mean(output_high[n_var, :]) - total_mean[n_var]) / (1 + total_std[n_var])),
                      ((numpy.mean(output_low[n_var, :]) - total_mean[n_var]) / (
                      1 + total_std[n_var]))]  # ! Calcul v-type
