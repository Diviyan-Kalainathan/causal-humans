...
def skel_const(mode, data_file, link_mat, ordered_var_names, pair_data=False, input_publicinfo='', types=False, causal_results='',
               flags=False, epsilon_diag=0.01):
        if not pair_data:
            if mode < 6:
                with open(data_file, 'rb') as pairs_file:
            ...
            #Value only per variable:
            pd.DataFrame(pair_data,columns=['Var','Val'])
