...
def compare_t_task( inputfolder, cluster_n, verbose=False):
    ...
    for var_1 in range(len(c_data_header) - 1):
        print(' ')
        sys.stdout.write('Cluster '+ str(cluster_n) + '----Var : '+str( c_data_header[var_1]) )
        sys.stdout.flush()
...
class data_formatter(threading.Thread):
    def __init__(self, inputfolder, cluster_n, pool, verbose=False):
    ...
    def run(self):
        compare_t_task(self.indir, self.num_cluster)
...
>>>>>>> c32f3b1832d068bd6330a3772457fadb3326fac8
    ...
    while not job_assigned:
        if Threadpools.numActive() < max_threads:
            jobs.append(data_formatter( inputfolder, cluster_n, Threadpools))
