...
def compare_t_task(var_1, c_data_header, cluster_data, inputfolder, cluster_n, verbose=False):
    ...
    for var_2 in range(var_1 + 1, len(c_data_header)):
        idx = var_2 - (var_1 + 1) ## Blank line appears?
        # print('.'),
...
if __name__ == '__main__':
    if prepare:
        ...
        while os.path.exists(inputfolder + 'cluster_' + str(cluster_n)):
            ...
            for var_1 in range(len(c_data_header) - 1):
                job_assigned = False
                ###IF No multithreadingfor var_2 in range(var_1 + 1, len(c_data_header)):
        =======
            #Load cluster data
            cluster_data= numpy.loadtxt(cluster_path, skiprows=1,delimiter=';')
            if verbose : print(cluster_data.shape)
        ...
        >>>>>>> c32f3b1832d068bd6330a3772457fadb3326fac8
                ...
                while not job_assigned:
                    if Threadpools.numActive() < max_threads:
                        ...
                        time.sleep(0.5)'''#If no multiprocessing
            print('Starting process '+str(cluster_n)+ ' ! ')
            p=Process(target=process_job,args=(cluster_n,))
            p.start()
            print('Process '+str(cluster_n)+ ' started ! ')

            processes.append(p)
            cluster_n += 1# Increment for the loop

        '''for thread in jobs:
            thread.join()'''#If no multiprocessing
