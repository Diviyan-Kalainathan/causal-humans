...
\begingroup
\thispagestyle{empty}
\AddToShipoutPicture*{\put(0,0){\includegraphics[scale=1.0]{couv.pdf}}} % Image background
\centering
\vspace*{5cm}
...
%\NOTE{afin de pouvoir en tirer des interprétations sur les mécanismes à l'origine des différences entre la satisfaction au travail des enquêtés et leur situation.\\
%? Mais comment tu sais que c'est des différences. J'ai reformulé.}
\par
...
%\NOTE{J'ai passé un coup de chic.}
...
%\NOTE{Un seul mot pour une même chose. Meme si c'est lourd en francais.}
\begin{figure}[!h]
  \centering
...
effet, la présence d'une corrélation traduit uniquement %juste : MS: non, trop informel
la "ressemblance
...
%\NOTE{Tu pourrais regrouper la fig. histogramme et le nombre de questions ds la table}
\begin{table}[!h]
\centering
...
%\end{figure}
%\NOTE{Tu peux supprimer cette figure}
%----------------------------------------------------------------------------------------
%	CHAPTER 2
%----------------------------------------------------------------------------------------
...
\begin{algorithm}[H]
 ...
 $\mathbf{W}\leftarrow \mathbf{W}[:p,:]$  \tcp{On tronque aux $p$ premiers vecteurs propres}
...
Les risques psychosociaux, l'organisation du travail et la santé n'apparaissent pas clairement dans les axes objectifs : ceci est du au faible nombre de questions objectives dans ces catégories %(Fig.\ref{fig:repar_objsubj})
ce qui réduit la variance expliquée par ces catégories.% Ainsi, les poids des catégories de variables sur les nouveaux axes subjectifs ressemble à un graphe complémentaire\NOTE{Pas compris cette phrase} sauf pour la "Santé", car elle ne comporte que 4 questions dans le questionnaire.
...
\For{$j \leftarrow 1$ \KwTo $r$}{
 ...
 \For{$i\leftarrow 1$ \KwTo $k$}{
  \BlankLine
   $CC[i] \leftarrow \textrm{Données}[rand((1,n_{exemples}))]$ \tcp{Centre choisi aléatoirement}
   \tcp{CC : Centres des clusters}
   \For{$x \leftarrow 1$ \KwTo $n_{exemples}$}{
     $ D(x)\leftarrow ||x-CC[i]||_2$
   }
   $CC[i] \leftarrow rand((1,n_{exemples}),weights=D(x)^2)$ \tcp{Centre choisi aléatoirement, avec une distibution de probabilité pondérée de $D(x)^2$}
 }
 \BlankLine
\tcp{K-means clustering}
\While{not converged}{
  \For{$x \leftarrow 1$ \KwTo $n_{exemples}$}{
      $T[X] \leftarrow \min\limits_{c\in CC}(x,c)$ \newline \tcp{Trouver/assigner le cluster le plus proche}
    
}
\For{$l \leftarrow 1$ \KwTo $k$}{
$CC[l] \leftarrow \frac{1}{|S_i^{(l)}|} \sum\limits_{x_j \in S_i^{(l}} x_j$\newline \tcp{Mise à jour des centres des clusters}
}}
\BlankLine
 $R[j] \leftarrow T$ 
...
\begin{figure}[!h]
\centering
  \includegraphics[scale=0.50]{Pictures/naf17_obj8.pdf}
...
\begin{figure}[!h]
\centering
  \includegraphics[scale=0.50]{Pictures/naf17_subj6.pdf}
...
\begin{figure}[!h]
\center
\includegraphics[scale=0.45]{Pictures/croisement/Matrice_xnormObj8subj6.pdf}
...
\begin{figure}[!h]
\center
\includegraphics[scale=0.45]{Pictures/croisement/Matrice_xObj8normsubj6.pdf}
...
\paragraph{}
Les différents clusters identifiés et leurs intersections peuvent être utilisés pour approfondir les liens entre QVT et d'autres facteurs tel que l'autonomie au travail des individus, un aspect de la QVT très étudié récemment\footnote{\href{https://www.anact.fr/management-du-travail-la-methode-anact}{cf. anact.fr/}}.% \NOTE{toutes les fois qu'on met "étudié récemment", il faut mettre une reference.}
...
%\NOTE{Toujours commencer par la motivation}
...
\begin{figure}[!h]
  \centering
  \subfloat[Graphe sans relation indirectes]{\includegraphics[width=4.5cm]{Pictures/graph_direct.pdf}}
  \hspace{1pt}
  \subfloat[Graphe avec relation indirectes]{\includegraphics[width=4.5cm]{Pictures/graph_indirect.pdf}}
...
\begin{figure}[!h]
\centering
 \includegraphics[width=12cm]{Pictures/graph-id-lptot.pdf}
...
%\NOTE{Toujours un chapitre de conc. et de perspectives. C'est aux perspectives qu'on juge le (futur) chercheur.}
